...
app.post('/auth/mfa/setup', requireFullAuth, async (req, res) => { try { const secret = generateSecret(); req.session.mfaSetupSecret = secret; const uri = generateURI({ issuer: 'CRYSTAL', label: req.user.email, secret }); res.json({ secret, qrCode: await QRCode.toDataURL(uri), uri }); } catch { res.status(500).json({ error: 'Could not create MFA setup' }); } });
app.post('/auth/mfa/enable', requireFullAuth, async (req, res) => { try { const secret = req.session.mfaSetupSecret; if (!secret) return res.status(400).json({ error: 'Start MFA setup first' }); if (!verifySync({ token: String(req.body.token || ''), secret }).valid) return res.status(400).json({ error: 'Invalid code' }); const backupCodes = Array.from({ length: 10 }, randomBackupCode); req.user.mfa = { enabled: true, secret: encrypt(secret), backupCodes: await Promise.all(backupCodes.map(c => argon2.hash(c))) }; await req.user.save(); delete req.session.mfaSetupSecret; res.json({ ok: true, backupCodes }); } catch { res.status(500).json({ error: 'Could not enable MFA' }); } });
