...
const userSchema = new mongoose.Schema({
  email: { type: String, lowercase: true, trim: true, index: true }, name: { type: String, trim: true }, passwordHash: String,
  providers: [{ name: String, id: String }], avatarUrl: String,
  emailVerified: { type: Boolean, default: false }, emailVerification: { tokenHash: String, expiresAt: Date }, passwordReset: { tokenHash: String, expiresAt: Date },
...
async function upsertOAuth(profile, provider) {
  const providerId = profile.id; let user = await User.findOne({ providers: { $elemMatch: { name: provider, id: providerId } } });
  const email = profile.emails?.[0]?.value?.toLowerCase(); if (!user && email) user = await User.findOne({ email });
  if (!user) user = new User({ email, name: profile.displayName || email || providerId, emailVerified: true });
  if (!user.providers.some(p => p.name === provider && p.id === providerId)) user.providers.push({ name: provider, id: providerId });
  user.emailVerified = true; if (profile.photos?.[0]?.value) user.avatarUrl = profile.photos[0].value; await user.save(); return user;
}
...
app.use(express.json({ limit: '32kb' })); app.use(express.urlencoded({ extended: false, limit: '32kb' }));
...
app.post('/auth/mfa/setup', requireFullAuth, async (req, res) => {
  try { const secret = generateSecret(); req.session.mfaSetupSecret = secret; const uri = generateURI({ issuer: 'CRYSTAL', label: req.user.email, secret }); res.json({ secret, qrCode: await QRCode.toDataURL(uri), uri }); }
  catch { res.status(500).json({ error: 'Could not create MFA setup' }); }
});
app.post('/auth/mfa/enable', requireFullAuth, async (req, res) => {
  try { const secret = req.session.mfaSetupSecret; if (!secret) return res.status(400).json({ error: 'Start MFA setup first' }); if (!verifySync({ token: String(req.body.token || ''), secret }).valid) return res.status(400).json({ error: 'Invalid code' }); const backupCodes = Array.from({ length: 10 }, randomBackupCode); req.user.mfa = { enabled: true, secret: encrypt(secret), backupCodes: await Promise.all(backupCodes.map(c => argon2.hash(c))) }; await req.user.save(); delete req.session.mfaSetupSecret; res.json({ ok: true, backupCodes }); }
  catch { res.status(500).json({ error: 'Could not enable MFA' }); }
});
